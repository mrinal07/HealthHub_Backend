...
// This is just a backup file for the routes
...
router.post("/upload-file", upload2.single("document"), async (req, res) => {
  if (!googleToken) {
...
async function createFolder(folderName, parentFolderId = null) {
  try {
    ...
    if (existingFolders.data.files.length > 0) {
      console.log("Parent/Root Folder already exists:", existingFolders.data.files[0]);
    ...
    if (parentFolderId) {
      ...
      if (existingFolders.data.files.length > 0) {
        console.log("Sub/Child Folder already exists:", existingFolders.data.files[0]);
        return existingFolders.data.files[0];
      }
